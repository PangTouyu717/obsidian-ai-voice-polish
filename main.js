...
var VolcengineSTT = class {
  ...
  async transcribe(audioBlob, audioFormat = "webm", audioEncoding = "opus") {
    ...
    if (response.json.code !== 0 && response.json.code !== 3e3) {
      throw new Error(
        `\u706B\u5C71\u5F15\u64CE STT \u4E1A\u52A1\u9519\u8BEF (code=${response.json.code}): ${(_a = response.json.message) != null ? _a : ""}`
      );
    }
    return {
      text: (_c = (_b = response.json.result) == null ? void 0 : _b.text) != null ? _c : "",
...
var _FloatingRecorder = class _FloatingRecorder {
  ...
  async startRecording() {
    ...
    try {
      ...
      this.plugin.setStatusBar("\u{1F399} \u5F55\u97F3\u4E2D...", true);
...
`;
  ...
  resetAll() {
    ...
    this.cleanupDurationTimer();
    document.removeEventListener("visibilitychange", this.onVisibilityChange);
    this.plugin.setStatusBar("\u{1F399} \u5F85\u547D", false);
    if (this.mode === "note") {
      this.statusEl.textContent = "\u7B14\u8BB0+\u97F3\u9891\u6A21\u5F0F";
    } else {
      this.statusEl.textContent = "\u70B9\u51FB\u5F55\u97F3";
...
var AiVoicePolishSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.createEl("p", {
      text: "\u{1F4A1} \u5F55\u5B8C\u5373\u51FA\u6587\u5B57\uFF0C\u6DA6\u8272\u5728\u540E\u53F0\u81EA\u52A8\u5B8C\u6210\uFF0C\u65E0\u9700\u7B49\u5F85\u3002",
      cls: "setting-item-description"
    });
  }
  /**
